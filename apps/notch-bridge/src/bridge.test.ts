...
describe("toResolution", () => {
  it("turns approve/always_allow/deny into bb approval decisions", () => {
    ...
    expect(
      toResolution(withSession, { id: "pint_1", action: "approve" }),
    ).toEqual({ decision: "allow_once", grantedPermissions: null });
    expect(
      toResolution(withSession, { id: "pint_1", action: "always_allow" }),
    ).toEqual({ decision: "allow_for_session", grantedPermissions: null });
    expect(toResolution(withSession, { id: "pint_1", action: "deny" })).toEqual(
      { decision: "deny" },
    );
  });

  it("downgrades always_allow when the session grant is not offered", () => {
    const onceOnly = approval(
      {
        kind: "command",
        itemId: "i",
        command: "ls",
  ...
  it("maps a chosen label back to the option value, and unknown text to free text", () => {
    ...
    expect(
      toResolution(question(), { id: "pint_1", action: "deny" }),
    ).toBeNull();
  });
});
